...
import { useCart } from '../../context/CartContext'; // 🚀 متصل به سبد خرید زنده
import { ShoppingBag, CreditCard, ShieldCheck, MapPin, Truck, ChevronLeft, Wallet } from 'lucide-react';
import Link from 'next/link';

export default function CheckoutPage() {
  const { cartItems, getCartTotal } = useCart(); // دریافت آیتم‌ها و مجموع مبلغ سبد خرید
  const [paymentMethod, setPaymentMethod] = useState('online'); // روش پرداخت: online یا wallet

  // اطلاعات تحویل فرضی (که در فازهای بعدی زنده از پروفایل یا استیت خوانده می‌شود)
  const userAddress = {
    name: "محسن عزیز",
    phone: "09123456789",
  ...
  const shippingCost = 45000; // هزینه ثابت ارسال اکسپرس سیب‌شاپ (تومان)
  const finalTotal = getCartTotal() + shippingCost; // مجموع نهایی فاکتور همراه با هزینه ارسال
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 direction-rtl antialiased flex flex-col justify-between">
      ...
      {/* 🏪 بدنه اصلی فاکتور خرید (کاملاً تراز با پهنا و حاشیه‌های هیرو باکس اصلی سایت) */}
      <main className="w-full px-4 md:px-8 py-4 md:py-6 flex-1">
        
        {/* مراحل خرید (مراحل گام‌به‌گام دیجی‌کالایی بالای صفحه) */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
          ...
          {/* 🔹 ستون سمت چپ: پیش‌فاکتور مالی و دکمه پرداخت نهایی (سایدبار) */}
          <aside className="lg:col-span-4 bg-white border border-slate-100 rounded-3xl p-5 shadow-3xs text-right sticky top-24">
            ...
            <div className="flex flex-col gap-3 pb-4 border-b border-slate-100">
              ...
              <div className="flex items-center justify-between w-full text-right">
                <span className="text-[11px] text-slate-400 font-bold">هزینه ارسال (اکسپرس سیب‌شاپ):</span>
                <div className="text-xs font-black text-slate-800 flex items-center gap-0.5">
            ...
            {/* دکمه اکشن هدایت به درگاه یا اتمام خرید */}
            <button 
